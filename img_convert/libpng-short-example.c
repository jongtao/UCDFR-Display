...
void process_file(char *outputFile)
{
	FILE *text = fopen(outputFile, "w");
	int k = 0;
	...
	for (y=0; y<height; y++) {
		...
		for (x=0; x<width; x++) {
			...
			printf("value %d, number: 0x%02X, k: %d, remainder: %d\n", !(ptr[0] && ptr[1] && ptr[2]), number,k , 7 - k%8);
